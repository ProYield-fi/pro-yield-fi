...
| 2 | **HL HIP-3 equity/commodity carry expansion** | xyz dex: 123 assets; **CL (crude) 167% APR with $183M OI → $9.1M size cap**, BRENTOIL 114%/$165M OI, ZHIPU 131%/$27M; stock names smaller (OI often <$1M). Energy/commodity names have REAL capacity — earlier '<$2M' reading was a units artifact (OI is in coins; must × mark price) | Short perp vs spot (or PM/pairs) collecting funding on equity/commodity/energy names | Same delta-neutral engine as our crypto carry; capacity is the binding constraint | **MONITOR + size-limit**. Add per-name OI to the carry scanner; cap = 5-10% of per-name OI. Equities coolest when markets chop. |
