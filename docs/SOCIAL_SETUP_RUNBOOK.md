...
**Re-verified 2026-09-25 (from the box):** Telegram @ProYieldFi live ✓ (og:title resolves) — wired in footer/nav/landing; X @ProYieldFi still listed (recheck rate-limited today; last full verify 09-24) — wired; LinkedIn still **free** (all three slugs 404) — awaits the phone step; Instagram recheck rate-limited today (last verified free 09-24); Reddit still unverifiable logged-out. Dead Discord invite **removed** from index.html structured data (sameAs = X + Telegram + GitHub). `xurl` still unauthed — awaiting the X developer-app keys. When new handles exist, verify publicly then wire the site in one commit (per the flow below).
...
**Copy kit:** all bios, first posts, cadence and guardrails are written and paste-ready in `docs/SOCIAL_COPY_KIT.md` — use that for the actual copy.
