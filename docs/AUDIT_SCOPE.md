...
| **Cross-artifact integrity audit** | `scripts/ecosystem_audit.js` (offline) | every artifact names the same canonical vault; no operating loop is silently stale; the deploy manifest is complete and non-zero; **every recycling ledger run matches the live policy split to the wei**; the web feed matches the reader's schema; website blend == scout's live blend; the operator alert queue carries no test-generated alerts. Checks whose artifact root is absent on the host SKIP (point `AUDIT_SCOUT_DIR` / `AUDIT_WEB_DIR` at a checkout to run them anywhere); `AUDIT_MODE=strict` turns FAILs into a non-zero exit (CI gate), default info mode reports operator-state drift without failing contract-correctness suites. |
