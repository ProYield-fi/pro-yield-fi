// scripts/create_insurance_multisig.js — create the dedicated insurance multisig
...
//   #2 a fresh ops signer generated on this box   -> ~/.proyield/insurance_signer.json
//   #3 a fresh backup signer generated on this box -> ~/.proyield/insurance_backup_signer.json
//
...
//      npx hardhat run scripts/create_insurance_multisig.js --network hyperTestnet
...
async function main() {
  ...
  const ops = loadOrCreateKey("insurance_signer.json", "insurance ops signer (2/3)",
    "testnet signer used by ops tooling; keep 0600");
  const backup = loadOrCreateKey("insurance_backup_signer.json", "insurance backup signer (3/3)",
  ...
  console.log("INSURANCE MULTISIG:", predicted);
}
main().catch((e) => { console.error("create_insurance_multisig error:", (e && e.stack) || e); process.exit(2); });